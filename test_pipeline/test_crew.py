#!/usr/bin/env python3
"""
Enhanced test file for the complete structured data processing pipeline.
...
def firecrawl_scrape_native(url: str, prompt: str = "", schema: dict = {}, test_mode: bool = True):
    ...
    try:
        ...
        def scrape_operation():
            return app.scrape_url(url, scrape_options)
        ...
        return {
            ...
            "documents": [link for link in result.get("linksOnPage", []) if any(ext in link.lower() for ext in ['.pdf', '.xlsx', '.xls', '.doc', '.docx'])],
            "metadata": {
                "url": url,
...
# ===================================================================
# SECTION 2: TEST CREW CLASS (Enhanced version)
# ===================================================================

class TestInverbotPipelineDato:
    """Enhanced test crew with complete pipeline functionality"""
    ...
    # ===================================================================
    # SECTION 3: HELPER METHODS (from crew.py)
    # ===================================================================
    ...
    def _identify_content_type(self, url: str, content: str) -> str:
        ...
        if "bolsadevalores.com.py" in url_lower or "bva" in url_lower:
            if "informes-diarios" in url_lower or "diario" in content.lower():
                return "BVA_Daily_Reports"
            elif "informes-mensuales" in url_lower or "mensual" in content.lower():
    ...
    def _process_bva_content(self, page_content: str, links: list, documents: list, structured_data: dict) -> tuple:
        """Process BVA content into structured database format"""
        metrics = {"records_created": 0, "processing_time": time.time()}
        
        try:
            # Extract emisores information
            emisores = []
            if "emisores" in page_content.lower():
                # Simple pattern matching for emisor names
            ...
            for i, doc_url in enumerate(documents[:3]):  # Limit to 3 for testing
                title = self._extract_title_from_link(doc_url)
                informes.append({
                    "id_informe": i + 1,
                    "id_emisor": 1,
                    "id_tipo_informe": 1,
            ...
            structured_data["Emisores"].extend(emisores)
            structured_data["Informe_General"].extend(informes)
            
            metrics["records_created"] = len(emisores) + len(informes)
            metrics["processing_time"] = time.time() - metrics["processing_time"]
            
            return structured_data, metrics
            
        except Exception as e:
            print(f"Error processing BVA content: {e}")
            return structured_data, metrics

    def _extract_title_from_link(self, url: str) -> str:
        """Extract title from document URL"""
        try:
            filename = url.split('/')[-1].replace('.pdf', '').replace('.xlsx', '')
    ...
    def extract_text_from_excel(self, excel_url: str) -> str:
    ...
    def normalize_data(self, structured_data: dict) -> dict:
    ...
    def validate_data(self, normalized_data: dict) -> dict:
    ...
    def chunk_document(self, text_content: str) -> dict:
    ...
    def run_test_pipeline(self):
        ...
        try:
            self.performance_metrics["pipeline_start_time"] = datetime.now().timestamp()
            
            # Execute the crew
            result = self.crew().kickoff()
            
            self.performance_metrics["pipeline_end_time"] = datetime.now().timestamp()
